...
def show_image_sample(X, y, task):
    ...
    for i in range(10):
        x_flair, x_t1, x_t1ce, x_t2, label = distort_imgs([X[:, :, 0, np.newaxis], X[:, :, 1, np.newaxis],
                                                           ...
                                                           y])  # [:,:,np.newaxis]])
        # print(x_flair.shape, x_t1.shape, x_t1ce.shape, x_t2.shape, label.shape)
        # (240, 240, 1) (240, 240, 1) (240, 240, 1) (240, 240, 1) (240, 240, 1)
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        for i in range(batch_size):
            if np.max(b_images[i]) > 0:
                vis_imgs_with_pred(b_images[i], b_labels[i], out[i], "samples/{}/train_{}.png".format(task, epoch))
                break
            elif i == batch_size - 1:
                vis_imgs_with_pred(b_images[i], b_labels[i], out[i], "samples/{}/train_{}.png".format(task, epoch))
        ...
        for i in range(batch_size):
            if np.max(b_images[i]) > 0:
                vis_imgs_with_pred(b_images[i], b_labels[i], out[i], "samples/{}/test_{}.png".format(task, epoch))
                break
            elif i == batch_size - 1:
                vis_imgs_with_pred(b_images[i], b_labels[i], out[i], "samples/{}/test_{}.png".format(task, epoch))
